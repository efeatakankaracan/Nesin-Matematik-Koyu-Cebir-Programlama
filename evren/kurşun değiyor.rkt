...
(define (evren-güncelle e)
 (alttan-cik (evren-nesne-e e)(alttan-cik (evren-nesne-d e)(alttan-cik (evren-nesne-c e)(alttan-cik (evren-nesne-b e)(evren-çatışma (evren-nesne-e e)(evren-çatışma (evren-nesne-d e)(evren-çatışma (evren-nesne-c e)
...
(define (evren-çiz-kaybet e)
  (place-text/v
   (v (/ (image-width BACKGROUND) 2) (/ (image-height BACKGROUND) 2))
   "Oyuna Başlamak İçin Boşluğa Basın"
   45
   "white"
   (rectangle 1080 (set-scale 1080) "solid" "black")))

(define (evren-çiz e)
  (cond [(>= (evren-puan e) 0)(place-text/v
   (v 100 30)
   (string-append "Puan:" (number->string (evren-puan e)))
   45
   "white"
   (nesne-çiz (evren-nesne-e e)
              (nesne-çiz (evren-nesne-d e)
                         (nesne-çiz (evren-nesne-c e)
                                    (nesne-çiz (evren-nesne-b e)
                                               (nesne-çiz (evren-nesne-a e)
                                                          (foldr nesne-çiz (evren-arkaplan e) (evren-b-liste e))))))))]
        [else (evren-çiz-kaybet e)]))
...
(define (evren-tuş e t)
  (evren (evren-arkaplan e)
         ...
         (evren-nesne-e e)
         (tuş-ateş e t)
         (cond [(< (evren-puan e) 0) (cond [(string=? t " ") 0] [else (evren-puan e)])] [else (evren-puan e)])))
...
(define (alttan-sek n imaj)
  (cond [
         (>=
          (+ (v-y (nesne-yer n))
             (/ (image-height (nesne-imaj n)) 2) -5)
          (image-height imaj))
         (nesne (nesne-imaj n)
                (v (random (+ 0 (/ (image-width (nesne-imaj n)) 2))
                           (- (image-width BACKGROUND)
                              (/ (image-width (nesne-imaj n)) 2)))
                   (+ 0 (/ (image-height (nesne-imaj n)) 2)))
                (v 0 (random 1 10))
                (nesne-ivme n)
                (nesne-can n)
                (nesne-puan n))][else n]))
...
(define (alttan-cik n e)
  (cond [(>= (+ (v-y (nesne-yer n))
                (/ (image-height (nesne-imaj n)) 2))
...
(define (ekrandan-cik n imaj)
  (cond
    [(< (v-x (nesne-yer n)) 0)
     (nesne (nesne-imaj n)
            (v (image-width imaj) (v-y (nesne-yer n)))
            (nesne-hız n)
            (nesne-ivme n)
            (nesne-can n)
            (nesne-puan n))]
    [(> (v-x (nesne-yer n)) (image-width imaj))
     (nesne (nesne-imaj n) (v 0 (v-y (nesne-yer n))) (nesne-hız n) (nesne-ivme n)
            (nesne-can n) (nesne-puan n))]
    [else n]))
...
(define (catisma-puan n k)
  (cond [(and
          ...
          (< (v-y (nesne-yer k)) (+ (image-width (nesne-imaj n)) (v-y (nesne-yer n))))) 10]
        [else 0]))
...
(define (catisma-oldu-mu? n k)
  (cond
    [(and
      (and
       (< (- (v-x (nesne-yer n)) (/ (image-width (nesne-imaj n)) 2)) (v-x (nesne-yer k)))
       (< (v-x (nesne-yer k)) (+ (v-x (nesne-yer n)) (/ (image-width (nesne-imaj n)) 2))))
      (< (v-y (nesne-yer k)) (+ (image-width (nesne-imaj n)) (v-y (nesne-yer n))))) true]
    [else false]))

(define (nesne-kursuna-carptı n k-liste)
  (cond [(not (empty? (filter (curry catisma-oldu-mu? n) k-liste )))
         (nesne (nesne-imaj n)
                (v (random (+ 0 (/ (image-width (nesne-imaj n)) 2))
                           (- (image-width BACKGROUND)
                              (/ (image-width (nesne-imaj n)) 2)))
...
(define (evren-helper e)
  (evren
   (evren-arkaplan e)
   (evren-nesne-a e)
   (nesne-kursuna-carptı (evren-nesne-b e) (evren-b-liste e))
   (nesne-kursuna-carptı (evren-nesne-c e) (evren-b-liste e))
   (nesne-kursuna-carptı (evren-nesne-d e) (evren-b-liste e))
   (nesne-kursuna-carptı (evren-nesne-e e) (evren-b-liste e))
   (evren-b-liste e)
   (evren-puan e)
   ))
...
(define (nesnelere-dokunuldu-mu? e n)
  (cond [(and
         ...
         (nesne
          (nesne-imaj n)
          (v (random (+ 0 (/ (image-width (nesne-imaj n)) 2))
                     (- (image-width BACKGROUND) (/ (image-width (nesne-imaj n)) 2)))
             (+ 0 (/ (image-height (nesne-imaj n)) 2)))
          (v 0 (random 1 10))
          (nesne-ivme n)
          (nesne-can n) 

...
(define player-image (bitmap "photos/spaceship.png"))
...
(define (draw-world w)
  (cond
    [(game-over w)
     (place-image (text "Oyun bitti! Tekrar oynamak için boşluk tuşuna bas.." 20 "white")
                  320 240
                  (rectangle 640 480 "solid" "black"))]
    [else (place-image (text (string-append "NinjaKedi!                                             Puan:"
                                            (number->string (world-score w))) 20 "white")
                       310 10   
                       (place-image (text "Hareket etmek için ok tuşlarını kullanın. Köpeğin üstüne atla ve yakuru yakala!" 12 "white")
                                    320 30
